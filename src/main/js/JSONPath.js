...
// PUBLIC METHODS
JSONPath.prototype = { 
	process : function(opts, expr, obj, callback, otherTypeCallback) {
	    if (!(this instanceof JSONPath)) {
	        try {
	            return new JSONPath().process(opts, expr, obj, callback, otherTypeCallback);
	        }
	        catch (e) {
	            if (!e.avoidNew) {
	                throw e;
	...
	evaluate : function (expr, json, callback, otherTypeCallback) {
    ...
    if (Array.isArray(expr)) {
        expr = this.toPathString(expr);
    }
    if (!expr || !json || allowedResultTypes.indexOf(this.currResultType) === -1) {
        return;
    }
    this._obj = json;
    var exprList = this.toPathArray(expr);
...
// PRIVATE METHODS
	...
	_getPreferredOutput : function (ea) {
    var resultType = this.currResultType;
    switch (resultType) {
    case 'all':
        ea.path = typeof ea.path === 'string' ? ea.path : this.toPathString(ea.path);
        return ea;
    case 'value': case 'parent': case 'parentProperty':
        return ea[resultType];
    case 'path':
        return this.toPathString(ea[resultType]);
    case 'pointer':
        return this.toPointer(ea.path);
    }
	},

	_handleCallback : function (fullRetObj, callback, type) {
    if (callback) {
        var preferredOutput = this._getPreferredOutput(fullRetObj);
        fullRetObj.path = typeof fullRetObj.path === 'string' ? fullRetObj.path : this.toPathString(fullRetObj.path);
        callback(preferredOutput, type, fullRetObj);
    }
	},

	_trace : function (expr, val, path, parent, parentPropName, callback) {
	...
	_walk : function (loc, expr, val, path, parent, parentPropName, callback, f) {
    var i, n, m;
    if (Array.isArray(val)) {
	...
	_slice : function (loc, expr, val, path, parent, parentPropName, callback) {
	...
	_eval : function (code, _v, _vname, path, parent, parentPropName) {
    ...
    if (code.indexOf('@path') > -1) {
        this.currSandbox._$_path = this.toPathString(path.concat([_vname]));
...
// For backward compatibility (deprecated)
	eval : function (obj, expr, opts) {
    return this.process(opts, expr, obj);
	}
};

exports.newJSONPath = function() {
	return new JSONPath();
}